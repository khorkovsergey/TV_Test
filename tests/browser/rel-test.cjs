...
(async () => {
  ...
  ok('названия совпадают с целевыми', JSON.stringify(IA.SECTIONS.map(s => s.label)) === JSON.stringify(SECTIONS));
  ...
  ok('в каждой есть «See everything»',
     [...home.d.querySelectorAll('.portal-nav .menu .nav-panel')].every(p => /See everything in/.test(p.textContent)));
  ...
  for (const p of ['/research', '/capital', '/trade', '/learn', '/community']) {
  ...
  console.log('\n[Capital] Живое, а не обещанное');
  const capStore = new Map([['watchlist', JSON.stringify(['BTCUSD', 'AAPL'])]]);
  const cap = await open('/capital', { store: capStore, wait: 2500 });
  ok('вотчлист показывает живые котировки', cap.d.querySelectorAll('#watchBody .mover-row').length === 2,
     String(cap.d.querySelectorAll('#watchBody .mover-row').length));
  const capEmpty = await open('/capital', { wait: 1800 });
  ok('пустое состояние предлагает действие', /Browse markets/.test(capEmpty.d.getElementById('watchBody').textContent));
  ...
  ok('главная — семь маршрутов задач + skip',
  ...
  ok('График: линия построена', ch.d.querySelectorAll('#priceLayer polyline').length === 1);
