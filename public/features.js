...
window.Features = (function () {
  ...
  const F = [
    {
      id: 'NEW-01', name: 'Beginner Mode + Guided Academy', shortName: 'Guided Academy',
      status: 'new', priority: 'strategic', icon: '◔',
    ...
    },
    {
      ...
      status: 'concept', priority: 'strategic', icon: '⇥',
    ...
    },
    {
      ...
      status: 'premium', priority: 'strategic', icon: '◆',
    ...
    },
    {
      ...
      solution: 'Licensed advisers verified by regulator and jurisdiction, matched to country, capital and question, with explicit control over what context is shared and a standardised written result.',
    ...
    { id: 'CORE-01', name: 'Task-based Home', shortName: 'Task Home', status: 'improved', priority: 'core', icon: '▤',
    ...
    { id: 'CORE-02', name: 'Progressive Complexity', shortName: 'Simple / Standard / Pro', status: 'improved', priority: 'core', icon: '▥',
    ...
    { id: 'CORE-03', name: 'Contextual Research Journey', shortName: 'Research Journey', status: 'improved', priority: 'core', icon: '▧',
    ...
    { id: 'CORE-04', name: 'Trust-first labelling', shortName: 'Trust-first', status: 'improved', priority: 'core', icon: '▨',
    ...
    { id: 'CORE-05', name: 'Value-first conversion', shortName: 'Value-first', status: 'improved', priority: 'core', icon: '▩',
  ...
  const BADGE = {
    new:      { label: 'NEW',      title: 'A new capability that works in this prototype' },
  ...
  const working = () => F.filter(f => f.status === 'new' || f.status === 'beta' || f.status === 'improved');
  const concepts = () => F.filter(f => f.status === 'concept' || f.status === 'premium');
  ...
  function badge(status, extraClass) {
    ...
    if (!b) return '';
    return `<span class="fbadge fbadge-${status} ${extraClass || ''}" title="${b.title}" role="note" aria-label="Status: ${b.label}">${b.label}</span>`;
  }

  function track(event, feature, props) {
    const rec = {
      feature_id: feature?.id, feature_status: feature?.status,
  ...
  function promo(id, surface, opts) {
    ...
    return `<a class="fpromo" href="${href}" data-fid="${f.id}" data-surface="${surface}">
      ...
      <span class="txt"><b>${o.title || f.shortName} ${badge(f.status)}</b>
        <span>${o.line || f.problem}</span></span>
      <span class="go">→</span></a>`;
  }

  return { ALL: F, BADGE, byId, onSurface, promo, strategic, core, working, concepts, badge, track, isShowcase, setShowcase };
})();
